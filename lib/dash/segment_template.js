...
shaka.dash.TimelineSegmentIndex = class extends shaka.media.SegmentIndex {
  ...
  */
  ...
  /**
   * Free all resources
   * Intended only to be called when entire segment index has been evicted
   */
  freeAll() {
  ...
  evict(time) {
    ...
    if (numToEvict > 0) {
      this.templateInfo_.timeline.splice(0, numToEvict);
      if (this.references.length >= numToEvict) {
        this.references.splice(0, numToEvict);
      }

      this.numEvicted_ += numToEvict;

      if (this.getNumReferences() === 0) {
        shaka.log.debug('Freeing all TSI resources');
        this.release();
        this.freeAll();
      }
    }
